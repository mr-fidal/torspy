...
async function findSubdomains(url, subFile, saveFile, saveDirectory) {
  ...
  const subdomains = fs.readFileSync(subFile, 'utf-8').split('\n');
  const foundSubdomains = [];

  for (const subdomain of subdomains) {
    const testUrl = `http://${subdomain.trim()}.${base_url}`;
    try {
  ...
  if (foundSubdomains.length > 0) {
    ...
    fs.writeFileSync(savePath, foundSubdomains.join('\n'), 'utf-8');
    console.log(`Found subdomains saved to ${savePath}`);
  } else {
